...
async function seed() {
  ...
  await prisma.mechanicStatusHistory.deleteMany();
  ...
  const categories = await Promise.all(
    SERVICE_CATEGORIES.map(cat => prisma.serviceCategory.create({ data: cat }))
  );
  console.log(`✅ Created ${categories.length} service categories`);

  // Create mechanics
  const mechanics = await Promise.all(
    MECHANIC_NAMES.map((name, i) => prisma.mechanic.create({
      data: {
        name,
        email: `mechanic${i + 1}@instantmechanic.com`,
        phone: `+1-555-${String(1000 + i).padStart(4, '0')}`,
        avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${name}`,
        status: randomItem([MechanicStatus.available, MechanicStatus.busy, MechanicStatus.offline]),
