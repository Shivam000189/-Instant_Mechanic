...
export class BookingService {
  ...
  async getBookingById(id: string) {
    ...
    if (!booking) throw new ApiError(404, 'BOOKING_NOT_FOUND', `Booking with ID '${id}' not found`);

    return booking;
  }

  async updateStatus(id: string, newStatus: BookingStatus, notes?: string) {
    const booking = await prisma.booking.findUnique({ where: { id } });
    if (!booking) throw new ApiError(404, 'BOOKING_NOT_FOUND', `Booking with ID '${id}' not found`);

    const validNextStatuses = VALID_TRANSITIONS[booking.status];
    if (!validNextStatuses.includes(newStatus)) {
      throw new ApiError(422, 'INVALID_STATUS_TRANSITION', 
        `Cannot transition from '${booking.status}' to '${newStatus}'`,
        { currentStatus: booking.status, requestedStatus: newStatus }
      );
    }
