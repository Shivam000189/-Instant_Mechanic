...
export class MechanicService {
  async getAllMechanics(params: { status?: MechanicStatus; search?: string }) {
    ...
    const stats = await prisma.mechanic.groupBy({
      by: ['status'],
      _count: { id: true }
    });
  ...
  async getMechanicById(id: string) {
    ...
    if (!mechanic) throw new ApiError(404, 'MECHANIC_NOT_FOUND', `Mechanic with ID '${id}' not found`);

    const performance = await prisma.booking.aggregate({
  ...
  async updateStatus(id: string, status: MechanicStatus, reason?: string) {
    ...
    await prisma.mechanicStatusHistory.create({
